...
include(joinpath(@__DIR__, "binary_var_block.jl"))
...
# Variables
@variable(LSP, x[i in machines, j in products, t in periods]>=0)
@variable(LSP, y[i in machines, j in products, t in periods], Bin)
@variable(LSP, s[j in products, t in periods]>=0)
...
# -------- RELAX AND FIX -----------
#blocks=partition_period(last(periods), 2)
blocks=partition_period(last(periods), 30)

rf_solution, rf_cost=relax_and_fix(LSP, blocks, binary_var_block)
